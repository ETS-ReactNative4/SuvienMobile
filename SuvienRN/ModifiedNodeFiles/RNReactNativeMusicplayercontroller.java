...
public class RNReactNativeMusicplayercontroller extends ReactContextBaseJavaModule {
    ...
    @ReactMethod
    public void preloadMusic(String desiredUri, Callback handler) {
        savedHandler = handler;
        SharedPreferences prefs = reactContext.getSharedPreferences("RNReactNativeMusicplayercontroller", 0);
        SharedPreferences.Editor editor = prefs.edit();
        editor.putString("RNReactNativeMusicplayercontroller", desiredUri);
        editor.commit();
        String savedUriString = prefs.getString("RNReactNativeMusicplayercontroller", "unknown"); //Get the uri that is saved there
        if (savedUriString != null && savedUriString != "unknown") { //As long as there's a uri string...
            ...
            if (metadatas.getType(0) == ReadableType.String) {//Give us the title, album, etc of the item
                ...
                boolean looping = "all" != "none";
                mediaPlayer = MediaPlayer.create(reactContext, audioUri); //Create a media player to play the song described.
